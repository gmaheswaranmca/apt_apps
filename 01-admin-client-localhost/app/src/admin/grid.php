<?php
    ...
    class grid
    {
		...
		public function DispArItems($query)
        {            
          //  db::connect();
            $rows = db::exec_sql($query);
            $found = false;
            //$r_count=db::num_fields($rows);
			$this->table="<div style='width:100%;font-size:10pt;'>";
            while($row=db::fetch($rows))
            {                
                ...
                foreach($this->_columns as $key=>$value)
                {
                    $this->table.="<span style='display:inline-block;border:1px solid silver;margin:5px;width:150px;'>&nbsp;
					<input $checked id='idchkgrd".$row[$this->id_column]."' type=checkbox name=chkgrd".$identity."[] value=".$row[$this->id_column]." /><label for='idchkgrd".$row[$this->id_column]."'>"
					.trim(strip_tags($this->FormatColumn($value,$row[$key])))."<label></span>";
                }
            ...
            if(!$found)
            {
                $this->table.="<div style='display:inline-block;min-width:250px;height:40px;border:1px solid silver;'>No Data</div>";
            }

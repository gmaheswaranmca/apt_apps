<?php 
	...
	class ApiController{
		...
		public function RunCode(){
			...
			$script = $this->q('script');
			$language = $this->q('language');
			$versionIndex = $this->q('versionIndex');
			
			$stdin = $this->q('stdin');
			
			if($clientId === 'aravindchennai' && $clientSecret==='heisattrichy'){
				$tc_out = $this->phpUtil->RunTechCode($script, $language, 
					$versionIndex, true, 
					$stdin);	// 3.1
				$p_useroutput=$tc_out['output']['output'];					
				$tc_out = array('output'=>$p_useroutput);	 

...
class ConductTestPageController	// change 1b
{
	...
	{			
		...
		$vmenu = $this->phpUtil->output_form(realpath(FS_DIR_CONDUCTTEST_PC.'../userAndModule/MenusView.php'));
		...
		if(!isset($_SESSION['txtLogin'])){
			...
			if($mdassessmentData[0]["user_quiz_status"] == 0){
				...
				$p_success = "0";
				$dbRes = $oConductTestDb->InsertUserQuiz($p_asg_id, $p_user_id, $p_status, $p_date, $p_success);
				$mdassessmentData = $oConductTestDb->QueryAssessesment($p_assignment_id, $p_user_id);
				//$_SESSION['asg_id']=$p_asg_id;
			}
			$mdassessment = array("assignment"=>$mdassessmentData, "user_id"=>$p_user_id, "user"=>$_SESSION['txtLogin'], "now_server"=>$this->phpUtil->Now());
			...
			$p_check_pass = true;	
			$oUserAndModuleDb = new UserAndModuleDb();
			$mdmenu = $oUserAndModuleDb->GetModules($p_user, $p_password, $p_imp_password, $p_check_pass);			
			//
			$outdata = array('IsLoggedIn'=>1,'vhtml'=>$vhtml,
					'vquestion_htm'=>$vquestion_htm,'vpager_htm'=>$vpager_htm,
					'vpager_qmatrix_htm'=>$vpager_qmatrix_htm, 'vmenu'=>$vmenu,					

<?php 
	...
	class ar6PhpMySqlDao
	{
		...
		private $m_db_host = DB_HOST;     
		private $m_db_name = DB_NAME; 	
		private $m_db_user = DB_USER;        
		private $m_db_pass = DB_PASS;        
		...
		{
			$this->m_connection_string = "mysql:host=".$this->m_db_host.";dbname=".$this->m_db_name;
		...
		public function db_write($p_sql, $p_param)
		{
			...
			$this->disconnect();
			//
			if($this->m_err === ""){return array('status'=>true,'last_id'=>$this->m_last_id);} else{return $this->m_err;}
		}
		...
		public function db_write_batch_bulk($p_sql, $p_param)
		{
			$this->connect();
			$this->m_err = "";
			//	
		...
		public function db_read_with_meta($p_sql, $p_param, $p_read_as=1)
		{
			...
			try {
				...
				$total_column = $v_stmt->columnCount();
				//var_dump($total_column);

				for ($counter = 0; $counter < $total_column; $counter ++) {
					$meta = $v_stmt->getColumnMeta($counter);
					$column[] = array('name' => $meta['name'], 
			...
			$this->disconnect();
			//
			$ret_res = array('data' => $v_result, 'fields' => $fields, 'column' => $column);
			if($this->m_err === ""){return $ret_res;} else{return $this->m_err;}
		}
	}
...
//txtMultiAns
    ...
    {        
        ...
        document.getElementById('divPager').innerHTML="Questions : "+data_arr[1];
    ...
    function ShowQst(x,y,id)
    {
        document.getElementById('divText').innerHTML="Please, wait ... ";
        MoveObject(x,y, "tblTip");
        $.post("modules/qst_previwer.php", {qst_preview:"yes", qst_id:id},
         function(data){
             //alert(data);             
    ...
    <table align="left" width="100%" border="0" cellpadding="0" cellspacing="0">
        <tr>
            <td style="display:<?php echo $timer_display ?>" align="left">
                ...
                Remaining Time <span id="tmrStart"></span>: <label id="lblTimer" style='font-size:15pt;'></label>
                </div>
            </td>        
            <td class="desc_text_bg2" align="right" >
                <div id="divPager" style="display:<?php echo $pager_display ?>;">
                    Questions : <?php echo $pager_html ?>
                </div>
            </td>                   
        </tr>
...
<table id="tblTip" style="display:none" cellspacing="0" cellpadding="0" border=0>
  <tr>
    <td align="right"><img border="0" src="tooltip_files/t1.gif" /></td>
  </tr>
	<tr >
		<td>		
			<table width="400px" cellspacing="0" cellpadding="0" style="border:0;border-width:2;border-color:#ED7C36" border=0 >
				<tr >
					<td style="width:1%" align=left valign=bottom ><img src="i/tool/tb_l.gif"></img></td>
				...
				<tr valign="top">
					...
					<td style="width:2px" align="left" ><img src="i/tool/tb_r_b.gif"></img></td>
				</tr>
			</table>
		</td>
	</tr>	
...
<script language="javascript">
        ...
        function Check() {
			/* M */
			vCliNow = new Date();
			vdNowSec = (Math.abs(vCliNow.getTime() - vCliStart.getTime()) / 1000) - 1;
			var va = parseInt(vQzSecs - vdNowSec);
			var vam = parseInt(va / 60);
			var vas = va % 60;
			//console.log(mins + "," + secs + "|" + vam + "," + vas);
			var vTampm = (vCliStart.getHours() >= 12 ? "PM" : "AM");
			var vThr = (vCliStart.getHours() > 12 ? vCliStart.getHours() - 12 : vCliStart.getHours());
			//vThr = (vThr <= 9 ? "0" : "") + vThr;
			var vTmin = vCliStart.getMinutes();
			//vTmin = (vTmin <= 9 ? "0" : "") + vTmin;
			var vStr = Pad(vThr) + ':' + Pad(vTmin) + ' ' + vTampm;
			document.getElementById('tmrStart').innerHTML = " (Start at " + vStr + ")";
			/* M End */
			
			if(!(mins == vam && secs == vas)) {mins = vam; secs = vas;}

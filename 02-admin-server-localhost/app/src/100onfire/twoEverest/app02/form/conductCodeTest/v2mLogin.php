...
?>
<?php 
if (!defined('FS_DIR_LOGIN_PC')) { /* FS: File System, PC: Page Controller */
define('FS_DIR_LOGIN_PC', __DIR__.'/');
}

$util_path = realpath(FS_DIR_LOGIN_PC.'../../sdk/ar6PhpUtil.php');	
$db_path = realpath(FS_DIR_LOGIN_PC.'ConductTestDb.php');
$module_db_path = realpath(FS_DIR_LOGIN_PC.'../userAndModule/UserAndModuleDb.php');	
...
class LoginPageController	// change 1b
{
	private $m_method = ''; 
	...
	/*2. dao-read*/
		...
		{			
			if(isset($_SESSION['txtLogin'])){
				$v_out = $this->phpUtil->output(1, 'Already Logged In', 'Already Logged In');
			}else{
				$v_out = $this->phpUtil->output(2, 'Yet To Login', 'Yet To Login');
			}

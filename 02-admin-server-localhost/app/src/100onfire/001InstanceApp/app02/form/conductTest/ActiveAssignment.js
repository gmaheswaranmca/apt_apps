	...
	this.QPgHtml=function()	// QPgHtml -> QListData -> ParsePgHtmlWListData
	{
		$.post("form/conductTest/v2mActiveAssignment.php",  { m: "html"}, 
			function(pData){
				...
				var v_rcode = pData.r.mdasscode;		
				if(v_rcode!=null)
					for(var i=0; i < v_rcode.length; i++){
						v_rcode[i].IsCode = true;
						v_rcode[i].IsQuizToStart = v_rcode[i].user_ass_status <= 1;
						v_rcode[i].IsQuizToContinue = v_rcode[i].user_ass_status == 1;
						var vAdjDatTime = null;
						if(v_rcode[i].user_ass_status >= 1)
							vAdjDatTime = JsController.AdjustTime(v_rcode[i].db_now, v_rcode[i].ass_start_date, v_rcode[i].total_duration);
						v_rcode[i].AdjDatTime = vAdjDatTime;
						v_rRes.push(v_rcode[i]);
					}	
				var v_rlink = pData.r.mdasslink;		
				if(v_rlink!=null)
					for(var i=0; i < v_rlink.length; i++){
						v_rlink[i].IsLink = true;
						v_rRes.push(v_rlink[i]);
					}	
							
				var rendered = Mustache.render(pData.r.vhtml,{assignment: v_rRes});

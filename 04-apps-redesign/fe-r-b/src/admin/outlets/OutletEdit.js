...
function OutletEdit() {
    ...
    const callToReadOutletById = async function(){ 
        ...
        const axiosResponse = await outletsService.readOne(params.id);
        const json = axiosResponse.data;
        setOutlet(json.data);
    }
    
    useEffect(() => { callToReadOutletById(); }, []);
    ...
    const doUpdateOutlet = function(event){
        if (!window.confirm(`Are you sure to create the outlet '${outlet.outletname}'?`)) {
            return;
        }
        const axiosResponse = outletsService.update(params.id,{
        ...
        alert('Outlet is updated successfully.');
        navigate("/admin/outlets/list")
        console.log(`${outlet.outletname} has been updated successfully`);
    }
            ...
            <div>
                ...
                <div className="for-group">
                    ...
                    <button type="button" class="btn btn-warning"
                     onClick={doUpdateOutlet}>Update Outlet</button>
                </div>
            </div>
        </>
    );
